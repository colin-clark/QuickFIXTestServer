package com.cep.darkstar.onramp.djnews;
...
public class DJNewsLogReader {

	final static Logger logger = Logger.getLogger("com.cep.darkstar.onramp.dj.DJNewsLogReader");
	...
	public void processMessages() throws IOException {
		...
		while (true) {
			// Get a list of NML files in the current directory
			FilenameFilter filter = new FilenameFilter() {
			    public boolean accept(File dir, String name) {
			        return name.endsWith(".NML");
			    }
			};
			String[] children = dir.list(filter);
			
			// If there are no NML files in the specified directory take a nap until some show up
			while (waitOnFile(children)) {
				waitOnFile();
				children = dir.list(filter);
			}
			
			// Parse each NML file and send it in!
	...
	private void waitOnFile() {
		logger.info("No new .NML files found in directory, waiting...");
		try {
			Thread.sleep(1000);
	...
	private void sendMessage(String headline, String story) {
		...
		try {
			...
			message.setEventName("DJHeadlines");
	...
	private String getStory(Document doc) {
		...
		Node body = docEle.getElementsByTagName("body").item(0);
		if(body != null) {
			NodeList bodyParts = body.getChildNodes();
			for (int i = 0; i < bodyParts.getLength(); i++) {
				Node part = bodyParts.item(i);
				if (part.getNodeName().equals("text")) {
					story = part.getTextContent();
					break;
				}
			}
		}
	...
	private String getHeadline(Document doc) {
		...
		if(body != null) {
			...
			for (int i = 0; i < bodyParts.getLength(); i++) {
				Node part = bodyParts.item(i);
				if (part.getNodeName().equals("headline")) {
					headline = part.getTextContent();
					break;
				}
			}
		}
	...
	private void setProperties(String configFile) throws Exception {
		...
		System.out.println("DJ News Log Reader is being configured using yaml config file " + configFile);
	...
	public static void main(String[] args) {
        DJNewsLogReader onramp = new DJNewsLogReader();
		try {
