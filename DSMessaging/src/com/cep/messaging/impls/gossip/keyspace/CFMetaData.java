...
import com.cep.messaging.impls.gossip.keyspace.marshal.UTF8Type;
...
public final class CFMetaData
{
    ...
    public final static String DEFAULT_ROW_CACHE_PROVIDER = "com.cep.messaging.impls.gossip.cache.ConcurrentLinkedHashCacheProvider";
    public final static String DEFAULT_COMPACTION_STRATEGY_CLASS = "com.cep.messaging.impls.gossip.keyspace.compaction.SizeTieredCompactionStrategy";
	...
	private void init()
    {
        ...
        try
        {
            rowCacheProvider             = GossipUtilities.newCacheProvider(DEFAULT_ROW_CACHE_PROVIDER);
        }
    ...
    public CfDef deflate()
    {
        ...
        cf.column_metadata = SerDeUtils.createArray(column_metadata.size(), ColumnDef.SCHEMA$);
    ...
    public static CFMetaData inflate(CfDef cf)
    {
        ...
        if (cf.row_cache_provider != null)
        {
            try
            {
                newCFMD.rowCacheProvider(GossipUtilities.newCacheProvider(cf.row_cache_provider.toString()));
            }
            catch (ConfigurationException e)
            {
    ...
    */
    ...
    public static CFMetaData fromThrift(CfDef cf_def) throws InvalidRequestException, ConfigurationException
    {
        ...
        if (cf_def.isSetRow_cache_provider()) { newCFMD.rowCacheProvider(GossipUtilities.newCacheProvider(cf_def.row_cache_provider)); }
    ...
    public void apply(CfDef cf_def) throws ConfigurationException
    {
        ...
        if (cf_def.row_cache_provider != null)
            rowCacheProvider = GossipUtilities.newCacheProvider(cf_def.row_cache_provider.toString());
        ...
        for (ColumnDef def : toAdd)
        {
            ...
            ColumnDefinition cd = new ColumnDefinition(def.name, 
                                                       dValidClass,
                                                       def.index_type == null ? null : IndexType.valueOf(def.index_type.toString()), 
        ...
        {
            compactionStrategyOptions = new HashMap<String, String>();
            for (Entry<String, String> e : cf_def.compaction_strategy_options.entrySet())
    ...
    public static CfDef convertToThrift(CFMetaData cfm)
    {
        ...
        List<ColumnDef> column_meta = new ArrayList<ColumnDef>(cfm.column_metadata.size());
    ...
    public static CfDef convertToAvro(CfDef def)
    {
        ...
        List<ColumnDef> columnMeta = new ArrayList<ColumnDef>();
        if (def.isSetColumn_metadata())
        {
            for (ColumnDef cdef : def.getColumn_metadata())
            {
