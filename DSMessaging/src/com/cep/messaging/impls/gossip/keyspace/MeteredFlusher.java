        ...
        try
        {
            for (ColumnFamilyStore cfs : ColumnFamilyStore.all())
            {
                ...
                int maxInFlight = (int) Math.ceil((double) (1 // live memtable
                                                            ...
                                                            + NodeDescriptor.getFlushWriters()
                                                            + NodeDescriptor.getFlushQueueSize())
                                                  / (1 + cfs.getIndexedColumns().size()));
                if (size > (NodeDescriptor.getTotalMemtableSpaceInMB() * 1048576L - flushingBytes) / maxInFlight)
                {
            ...
            if (flushingBytes + liveBytes <= NodeDescriptor.getTotalMemtableSpaceInMB() * 1048576L)
                return;
            ...
            {
                flushingBytes = countFlushingBytes();
                if (liveBytes + flushingBytes <= NodeDescriptor.getTotalMemtableSpaceInMB() * 1048576L || sorted.isEmpty())
                    break;
