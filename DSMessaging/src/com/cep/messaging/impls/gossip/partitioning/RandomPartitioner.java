...
import com.cep.messaging.impls.gossip.partitioning.token.BigIntegerToken;
...
import com.cep.messaging.impls.gossip.util.GuidGenerator;
...
public class RandomPartitioner implements IPartitioner<BigIntegerToken> {
	...
	public Token midpoint(Token ltoken, Token rtoken) {
		...
		Pair<BigInteger, Boolean> midpair = GossipUtilities.midpoint(left, right, 127);
	...
	public BigIntegerToken getRandomToken() {
		BigInteger token = GossipUtilities.hashToBigInteger(GuidGenerator.guidAsBytes());
		if (token.signum() == -1) {
	...
	public BigIntegerToken getToken(ByteBuffer key) {
		...
		return new BigIntegerToken(GossipUtilities.hashToBigInteger(key));
	}
	...
	public Map<Token, Float> describeOwnership(List<Token> sortedTokens) {
		...
		// n-case
			...
			// The start token's range extends backward to the last token, which
			// is why both were saved above.
