...
public class DJNews extends AbstractDTO implements IDTO {
	static Logger logger = Logger.getLogger("com.cep.darkstar.DTO.DJNews");
	...
	public void doFunction(EventObject e) {
		...
		String story = null;
		if (logger.isDebugEnabled()) {
			logger.debug("***DJNews DTO persisting Event");
		}
		save(e);
		if (e.has("story")) {
			try {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("***DJNews DTO Sending Event");
		}
		CEPEventsSendRunnable.handleEventObject(e);
		if (story != null) {
			try {
				if (logger.isDebugEnabled()) {
					logger.debug("***DJNews DTO analyzing Story");
				}
				Set<String> symbols = analyzer.processDoc(story);
				if (logger.isDebugEnabled()) {
					logger.debug("***DJNews DTO analysis returned symbols " + symbols.toString());
				}
	...
	private void sendNamedEntity(String symbol, String guid) throws JSONException {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("DJNews DTO Sending Event " + event);
		}
